...
const TasksPage = () => {
  const { viewMode, setViewMode, tasks, currentProject, projects } = useTaskStore();
  ...
  const filteredTasks = tasks.filter(task => {
    if (currentProject && task.projectId !== currentProject) return false;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Overdue', value: filteredTasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'done').length, color: 'red' },
        ].map((stat, index) => (
          <motion.div
            key={index}
            className="card p-6"
